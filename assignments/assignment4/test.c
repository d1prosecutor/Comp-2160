 ...
 * GLOBAL VARIABLES
 *******************************/
unsigned char buffer1[MEMORY_SIZE];
unsigned char buffer2[MEMORY_SIZE];
    ...
    if (ref > 0)
    {
        ...
        while ((NULL != curr) && objectNotFound)
        {
            ...
            if (ref == curr->referenceID)
            {
                ...
                result = (void *)&(currentBuffer[curr->startAddress]);
                objectNotFound = 0;
            }
            curr = curr->next;
        }
...
static void copyMemBlock(void *emptyMemPool, Node *memBlock)
{
    uint8_t *addressToCopy = &(currentBuffer[memBlock->startAddress]);
    uint8_t *tempEmptyPool = (uint8_t *)emptyMemPool;

    // Update the start address of each node
    memBlock->startAddress = freePtr;

    Ref index = 0;
    while (index < memBlock->numBytes)
    {
        tempEmptyPool[freePtr++] = addressToCopy[index];
        index++;
    }
